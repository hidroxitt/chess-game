...
function updatePlayerDisplay() {
  ...
  const whiteIcon = whitePlayer.querySelector("i");
  const blackIcon = blackPlayer.querySelector("i");

  if (inTurn === "white") {
    whiteIcon.className = "fa-solid fa-circle-user";
    blackIcon.className = "fa-regular fa-circle-user";
    whitePlayer.classList.add("active");
    blackPlayer.classList.remove("active");
  } else {
    blackIcon.className = "fa-solid fa-circle-user";
    whiteIcon.className = "fa-regular fa-circle-user";
...
const initialTime = parseInt(localStorage.getItem("initialTime"), 10) || 600;
const incrementTime = parseInt(localStorage.getItem("incrementTime"), 10) || 0;
...
function startWhiteTimer() {
  whiteTimerInterval = setInterval(() => {
    whiteTimeLeft--;
    document.getElementById("white-timer").innerText = `${formatTime(
      whiteTimeLeft
    )}`;
...
function startBlackTimer() {
  blackTimerInterval = setInterval(() => {
    blackTimeLeft--;
    document.getElementById("black-timer").innerText = `${formatTime(
      blackTimeLeft
    )}`;
...
function changeTurn() {
  ...
  updatePlayerDisplay();
  if (inTurn === "black") {
    stopTimer(whiteTimerInterval);
    whiteTimeLeft += incrementTime;
    startBlackTimer();
  } else {
    stopTimer(blackTimerInterval);
...
function checkKingCaptured() {
  const whiteKing = globalPiece.white_king; // Truy cập quân vua trắng
  const blackKing = globalPiece.black_king; // Truy cập quân vua đen

  if (!whiteKing || !whiteKing.current_position) {
    alert("Black wins! The white king has been captured.");
    endGame("black");
  }

  if (!blackKing || !blackKing.current_position) {
